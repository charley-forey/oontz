...
function tick(now){
  ...
  var th = THEMES[S.theme] || THEMES.acid, f = frameFor(eng, now / 1000), draw = MODES[S.mode];
  if(!draw) return;
...
function apply(v){                               // a stored/song look, validated field by field
  if(!v) return;
  if(v.mode === "off" || MODES[v.mode]) S.mode = v.mode;
  if(THEMES[v.theme]) S.theme = v.theme;
  Object.keys(PARAMS).forEach(function(k){ var x = parseParam(k, v[k]); if(x != null) S[k] = x; });
}

var VIZ = {
  THEMES: THEMES, MODES: MODES, parseParam: parseParam, mix: mix, clockPhase: clockPhase, bands: bands, upcoming: upcoming,
